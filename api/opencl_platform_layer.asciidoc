...
| {CL_DEVICE_QUEUE_ON_HOST_PROPERTIES_anchor}^9^
...
4::
    ...
    `CL_VENDOR_ID_<vendor>`. The vendor ID will be reserved only once this merge
    request has been accepted. Please do not try to reserve vendor IDs unless
    you are making a good faith effort to develop a conformant OpenCL
    implementation and require one for that purpose.
